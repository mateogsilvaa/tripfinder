...
test.describe("la ventana que se amplía", () => {
  ...
  test("el nav lleva a las cinco páginas, sin tener que lanzar nada", async ({ page }) => {
    await page.goto("/index.html", { waitUntil: "domcontentloaded" });
    const hrefs = await page.locator(".zona").evaluateAll((as) => as.map((a) => a.getAttribute("href")));
    expect(hrefs).toEqual(["./", "buscar.html", "seguimientos.html", "trenes.html", "interrail.html"]);
