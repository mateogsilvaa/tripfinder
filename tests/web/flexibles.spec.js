...
test.describe("el encargo", () => {
  test("un mes entero manda sus dos extremos y no se limita a findes", async ({ page }) => {
    ...
    expect(p.fechas.desde).toBe(`${mes}-01`);
    // El último día del mes, sea 28, 30 o 31.
    expect(p.fechas.hasta.startsWith(mes)).toBe(true);
    expect(new Date(p.fechas.hasta).getMonth()).toBe(new Date(`${mes}-01`).getMonth());
    expect(p.weekend).toBe("no");
    expect(p.fechas.depart).toBe("");
  });
  ...
  test("un mes de findes manda la misma ventana, pero solo findes", async ({ page }) => {
    ...
    expect(p.fechas.desde).toBeTruthy();
    expect(p.fechas.hasta).toBeTruthy();
  });
  ...
  test("un tramo manda los dos días marcados", async ({ page }) => {
    ...
    const p = (await enviar()).client_payload;
    expect(p.fechas.desde).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    expect(p.fechas.hasta > p.fechas.desde).toBe(true);
  ...
  test("las fechas exactas siguen yendo como fechas exactas", async ({ page }) => {
    ...
    const p = (await enviar()).client_payload;
    expect(p.fechas.depart).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    expect(p.fechas.return_date).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    expect(p.fechas.desde).toBe("");
  });
