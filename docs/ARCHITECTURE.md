...
| `mundo.yml` | `repository_dispatch: mundo` | El mapa de países de una cuenta: lo escribe en `data/mundos/<uid>.json` y rehace `data/mundos/index.json`. Guardarlo es **publicarlo** —el repositorio es público y `data/` se copia entero a Pages—, que es justo lo que permite ver el mapa de los demás; apagar el interruptor borra el fichero. |
