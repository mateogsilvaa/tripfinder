...
| `interrail.yml` | `repository_dispatch: interrail` | Busca alojamiento entero —nada de habitaciones— en cada parada de una ruta de Interrail, en un solo encargo: uno por parada perdería los del medio, porque GitHub solo guarda una ejecución en espera por cola. Escribe `data/stays/ir-*.json`. |
